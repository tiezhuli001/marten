...
class SleepCodingWorkerService:
    ...
    def poll_once(
        ...
        for issue in discovered:
            ...
            with closing(self._connect()) as connection:
                self._record_discovered_issue(connection, repo, request.worker_id, issue)
                if not self._is_issue_eligible(issue):
                    self._mark_claim_status(connection, repo, issue.issue_number, "skipped")
                elif not self._is_ready_for_retry(connection, repo, issue.issue_number):
                    self._mark_claim_status(connection, repo, issue.issue_number, "retry_pending")
            ...
            try:
                ...
                with closing(self._connect()) as update_connection:
                    ...
                    update_connection.commit()
        with closing(self._connect()) as connection:
            claims = self._list_claims(connection, repo)
    ...
    def list_claims(self, repo: str | None = None) -> list[SleepCodingWorkerClaim]:
        ...
        with closing(self._connect()) as connection:
            ...
            connection.commit()
            return self._list_claims(connection, target_repo)
    ...
    def _acquire_lease(
        ...
        connection.execute(
            ...
            WHERE repo = ? AND issue_number = ?
            """,
            (worker_id, lease_expires_at, timestamp, repo, issue_number),
        )
    ...
    def _record_failure(
        ...
        next_retry = self._format_dt(self._utcnow() + timedelta(seconds=self.retry_backoff_seconds))
        ...
        connection.execute(
            ...
            WHERE repo = ? AND issue_number = ?
            """,
            (status, worker_id, retry_count, next_retry, error_text[:500], repo, issue_number),
        )
