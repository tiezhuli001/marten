...
class MainAgentService:
    ...
    def intake(self, payload: MainAgentIntakeRequest) -> MainAgentIntakeResponse:
        ...
        draft, usage = self._build_issue_draft(payload)
    ...
    def _build_agent_descriptor(self) -> AgentDescriptor:
        return AgentDescriptor(
            agent_id="main-agent",
            workspace=self.settings.resolved_main_agent_workspace,
            skill_names=self.settings.resolved_main_agent_skills,
            mcp_servers=self.settings.resolved_main_agent_mcp_servers,
            model_profile=self.settings.resolved_main_agent_model_profile,
            system_instruction=(
                "Convert user requests into executable GitHub issues that Ralph can execute through the sleep-coding workflow."
            ),
        )
